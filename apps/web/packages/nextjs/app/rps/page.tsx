...
function CompactHistoryCard({
  ...
  const stateNames = ["创建", "加入", "锁定", "结算", "过期"];
  ...
  // 计算输赢状态
  const isWinner = isResolved && matchData.winner.toLowerCase() === myAddress.toLowerCase();
  const isLoser = isResolved && isMyMatch && matchData.winner !== ethers.ZeroAddress && !isWinner;
  const isDraw = isResolved && matchData.winner === ethers.ZeroAddress;
  ...
  const formatTimeRemaining = (deadline: number) => {
    ...
    if (diff > 0) {
      ...
      if (absDiff < 60) return `${absDiff}秒前过期`;
      if (absDiff < 3600) return `${Math.floor(absDiff / 60)}分钟前过期`;
      if (absDiff < 86400) return `${Math.floor(absDiff / 3600)}小时前过期`;
      return `${Math.floor(absDiff / 86400)}天前过期`;
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 flex gap-6">
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="bg-white rounded-2xl border-4 border-yellow-400 shadow-xl p-6">
            <h2 className="text-2xl font-black text-black mb-4">🎮 游戏模式</h2>
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="font-black text-lg">练习模式</div>
                <div className="text-xs font-semibold opacity-70">免费对战</div>
              </button>
              <button
              ...
              >
                <div className="text-3xl mb-2">💰</div>
                <div className="font-black text-lg">真实模式</div>
            ...
            {selectedMode === 1 && (
              <div className="mt-4 p-4 rounded-xl bg-yellow-200 border-2 border-yellow-400">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-black/70 font-semibold">匹配池</span>
                  <span className="text-black font-black">{pendingCount} 个对局</span>
                </div>
              </div>
            )}
          </div>
          ...
          {/* 出手区域 */}
          <div className="bg-white rounded-2xl border-4 border-yellow-400 shadow-xl p-6">
            <h2 className="text-2xl font-black text-black mb-6">✊✋✌️ 选择出手</h2>
            <div className="grid grid-cols-3 gap-6">
              {[
                { move: 0 as Move, icon: "✊", name: "石头", color: "red" },
                { move: 1 as Move, icon: "✋", name: "布", color: "blue" },
                { move: 2 as Move, icon: "✌️", name: "剪刀", color: "green" },
          ...
          {matchState && (
            <div className="bg-white rounded-2xl border-4 border-yellow-400 shadow-xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-2xl font-black text-black">📊 对局状态</h2>
                <button
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between p-3 bg-yellow-100 rounded-lg border-2 border-yellow-300">
                  <span className="text-black/70 font-semibold">状态</span>
                  <span className="text-black font-black">{matchState.stateName}</span>
                </div>
                <div className="flex justify-between p-3 bg-yellow-100 rounded-lg border-2 border-yellow-300">
                  <span className="text-black/70 font-semibold">玩家A</span>
                ...
                {matchState.playerB !== ethers.ZeroAddress && (
                  <div className="flex justify-between p-3 bg-yellow-100 rounded-lg border-2 border-yellow-300">
                    <span className="text-black/70 font-semibold">玩家B</span>
                ...
                {matchState.isResolved && matchState.winner !== ethers.ZeroAddress && (
                  <div className="flex justify-between p-3 bg-green-100 rounded-lg border-2 border-green-400">
                    <span className="text-green-700 font-bold">🏆 获胜者</span>
        ...
        <div className="w-80 space-y-4">
          <div className="bg-white rounded-2xl border-4 border-yellow-400 shadow-xl p-6 sticky top-24 max-h-[calc(100vh-8rem)] flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-black text-black">📜 历史</h2>
              <button
                onClick={async () => {
                  if (address) {
